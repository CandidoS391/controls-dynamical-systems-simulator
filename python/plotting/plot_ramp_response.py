...
with open("output/ramp_response_euler.csv", mode="r") as euler_file:
  next(euler_file)
...
with open("output/ramp_response_rk4.csv", mode="r") as rk4_file:
  next(rk4_file)
...
plt.title("Ramp Input Response: Position vs Time")
plt.xlabel("Time")
...
plt.title("Ramp Input Response Phase Portrait")
...
plt.figure(1)
plt.savefig("output/ramp_position_vs_time.png")

plt.figure(2)
plt.savefig("output/ramp_phase_portrait.png")
